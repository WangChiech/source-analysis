...
export function init(
  ...
  function createRmCb(childElm: Node, listeners: number) {
    // 先缓存，所有 remove 钩子函数都执行过后才真正执行删除操作
    ...
    for (; startIdx <= endIdx; ++startIdx) {
      ...
      if (ch != null) {
        ...
        if (isDef(ch.sel)) {
          ...
          // 确保调用完所有模块的 remove 钩子之后，再一次调用 rm() 可执行 DOM 删除
          listeners = cbs.remove.length + 1;
          // 先缓存删除 DOM 的操作，所有 remove 钩子函数都执行过后才真正执行删除操作
  ...
  function patchVnode(
    ...
    if (isUndef(vnode.text)) {
      if (isDef(oldCh) && isDef(ch)) {
        // 新 vnode 有子节点，旧 vnode 有子节点
        if (oldCh !== ch) updateChildren(elm, oldCh, ch, insertedVnodeQueue);
      } else if (isDef(ch)) {
        // 新 vnode 有子节点，旧 vnode 有文本节点，或什么也没有
        if (isDef(oldVnode.text)) api.setTextContent(elm, "");
        addVnodes(elm, null, ch, 0, ch.length - 1, insertedVnodeQueue);
      } else if (isDef(oldCh)) {
        // 新 vnode 无子节点，旧 vnode 有子节点
        removeVnodes(elm, oldCh, 0, oldCh.length - 1);
      } else if (isDef(oldVnode.text)) {
        // 新 vnode 无子节点，旧 vnode 有文本节点
  ...
  return function patch(
    ...
    let i: number, 
      elm: Node, 
      parent: Node;
    // 存放已创建出 DOM 节点的，并且存在 insert 钩子的 vnode
    ...
    if (sameVnode(oldVnode, vnode)) {
      ...
      // 根据 VNode 创建 Dom，存入 VNode.elm
      createElm(vnode, insertedVnodeQueue);
      // 判断旧 vnode 上的 DOM 父节点是否存在
      if (parent !== null) {
        // 父节点存在
        // 将新 vnode.elm 上挂载的 DOM 插入到视图
